...
#define CODE_H_VERSION_MN 2

// .~^~.~^~.~^~.~^~.~^~.

// Тип контекста
enum class ContextType {
	ConditionalStatement, // Оператор условия
	LoopStatement, // Оператор цикла
...
class Code {
	public:
		...
		VarType expression(std::vector<Node> expr, std::vector<char*> _prefix, VarType expected_type = VarType::None, bool is_func_call = false, uint8_t _max_st = 0);
