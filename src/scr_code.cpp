...
const char* OpTypeToString(OpType t) {
...
void Code::loadLibs(std::vector<lib_struct> &libs) {
	...
	for (auto i = libs.begin(); i != libs.end(); i++) {
		name = i->name;
		functionsCount = i->size;
		st.name = name;
		st.lib = (lib_reg_c*)malloc(sizeof(lib_reg_c) * functionsCount);
		st.size = functionsCount;

		for (int n = 0; n < functionsCount; n++) {
	...
	DEBUGF("Libraries loaded. Built-in functions: %d\n", cpp_libs_id);
}

int FindVarType(VarType type) {
	for (int i = 0; i < sizeof(var_types_info) / sizeof(var_types_info[0]); i++) {
		if ((uint16_t)var_types_info[i].type == type.vt.value)
			return i;
	}
	return -1;
}
...
const char* varTypeToString(VarType t) {
	for (int i = 0; i < sizeof(var_types_list) / sizeof(var_types_list[0]); i++) {
		if (var_types_list[i].type == (VarType::Std)t.vt.value)
...
// 	PRINT_ERROR("Переменная '%.64s' уже объявлена", name);
// 	internalException(1);
// }

static void errorNotDeclared(char *name) {
	PRINT_ERROR("Переменная '%.64s' не была объявлена", name);
	internalException(1);
}

static void errorDeclaredFunction(char *name) {
	PRINT_ERROR("Функция '%.64s()' уже была объявлена", name);
	internalException(1);
}

static void errorNotDeclaredFunction(char *name) {
	PRINT_ERROR("Функция '%.64s()' не была объявлена", name);
	internalException(1);
}

static void errorArrayType(char *name) {
	PRINT_ERROR("Переменная '%.64s' не является массивом", name);
	internalException(1);
}

static void errorFunctionArgsCount(char *name, size_t got, size_t expected) {
	PRINT_ERROR("Неверное число аргументов в вызове функции '%.64s()': ожидалось %d, получено %d", name, expected, got);
	internalException(1);
}

static void errorTooManyFunctionArgs(char *name) {
	PRINT_ERROR("Слишком много аргументов в вызове функции '%.64s()'", name);
	internalException(1);
}

static void errorWrongFunctionArgType(unsigned int num, char *name) {
	PRINT_ERROR("Неверный тип аргумента #%d в вызове функции '%.64s()'\n", num, name);
	internalException(1);
}

static void errorReturnType(VarType type) {
	PRINT_ERROR("Функция не может вернуть значение с типом '%.64s'", varTypeToString(type));
	internalException(1);
}

static void errorNoReturnValue() {
	PRINT_ERROR("Функция в выражении должна возвращать значение");
	internalException(1);
}

static void errorNoExpressionValue() {
	PRINT_ERROR("Отсутствует выражение");
	internalException(1);
}

static void errorTypeConvertion(VarType::Std type, VarType::Std expected) {
...
static void errorWrongPrefix(char *name, char *pref) {
	PRINT_ERROR("'%.64s' не принадлежит к '%.64s'", name, pref);
	internalException(1);
}

static void errorNotFoundGotoLabel(char *l) {
	PRINT_ERROR("Не найдена метка ::%.64s::", l);
	internalException(1);
}

static void errorNoReturnStatementInFunction(char *name) {
	PRINT_ERROR("Оператор 'return' отсутвует или недостижим в функции '%.64s'", name);
	internalException(1);
}

// Новое окружение
void Code::newScope(char *name) {
	DEBUGF("Code::NewScope\n");
...
void Code::closeScope() {
	DEBUGF("Code::CloseScope\n");
	FuncState *temp;
	if (current_scope != nullptr) {
		char *name = current_scope->name;
		int l = findFunction(name);
		if (l != -1) {
			VarType expected = declared_functions[l].args[0].type; // ожидаемый тип
			if (expected.vt.value != (uint16_t)VarType::Std::None && current_scope->got_return == false)
...
// Добавляет структуру
size_t Code::addDefState(DefState state) {
	structs.push_back(state);
	return structs.size() - 1;
}

/* Поиск структуры с именем 'name'
 * Возвращает номер структуры
*/
int Code::findDefState(char *name) {
	int n = 0;
	for (auto i = structs.begin(); i != structs.end(); i++) {
		if (compareStrings(i->name, name))
			return n;
		n++;
	}
	return -1;
...
// Добавляет поле в структуру
size_t Code::defStatePushField(FieldState field) {
	if (structs.size() > 0) {
		structs.back().body.push_back(field);
		return structs.back().body.size() - 1;
	}
	return 0;
}

/* Поиск поля с именем 'varname' в структуре #stateid
 * Возвращает номер поля
*/
int Code::defStateFindField(int stateid, char *varname) {
	DefState *ds = &(structs.at(stateid));
	int n = 0;
	for (auto i = ds->body.begin(); i != ds->body.end(); i++) {
		if (compareStrings(i->name, varname))
			return n;
		n++;
	}
	return -1;
...
VarType Code::getFunctionReturnType(char *name, char *pref) {
	...
	if (l != nullptr) { // *если встроенная функция
		...
		if (l2 == -1) { // *если не объявлена
			...
			return declared_functions[l2].args[0].type;
		}
	}
...
int Code::findLocal(char *name) {
	...
	{
		n--;
		if ((IsConstVarType(locals[n].type) // если константа
...
VarType Code::getVariableType(char *name) {
	int l = findLocal(name);
	if (l != -1)
		return locals[l].type;
	l = findGlobal(name);
	if (l != -1)
		return globals[l].type;
...
bool Code::variableHasParent(char *name) {
	bool is_local;
...
int Code::declareVar(char *name, VarType type, bool is_local, int parent) {
	VarState vs;
	if (is_local) {
		locals.push_back(vs);
		locals.back().name = name;
		locals.back().type = type;
		locals.back().data = 0;
		locals.back().id = current_scope->locals_count;	
		locals.back().parent = parent;
		if (subcontexts.size() > 0 &&
			(ContextType)subcontexts.back().type != ContextType::FunctionDeclaration)
		{
		...
		globals.back().type = type;
		globals.back().data = 0;
		globals.back().id = globals.size() - 1;
		globals.back().parent = parent;
...
int Code::declareLocal(char *name, VarType type) {
	//int l = findLocal(name);
	//if (l == -1) { // если не объявлена
		return declareVar(name, type, true, -1);
	//} else {
	//	errorDeclared(name);
	//}
	//return 0;
...
void Code::loadGlobal(char *name) {
	...
	if (l == -1) // *если не объявлена
		...
		codeLoadVariable(globals[l].type, false, l);
}
...
void Code::storeGlobal(char *name) {
	...
	if (l == -1)
		...
		codeStoreVariable(globals[l].type, false, l);
}
...
int Code::declareGlobal(char *name, VarType type) {
	...
	// // если не объявлена
	// if (l == -1) {
	 	return declareVar(name, type, false, -1);
	// } else {
	// 	errorDeclared(name);
	// }
	// return 0;
...
VarType Code::unOpType(OpType optype, VarType vtype) {
	DEBUGF("UnOpType ");
	VarType type = VarType::Std::None;
	for (int i = 0; i < sizeof(unop_types_info) / sizeof(unop_types_info[0]); i++) {
		if (unop_types_info[i].type == optype) {
			if (vtype.vt.value == (uint16_t)VarType::Std::Int) {
				type = VarType::Std::Int;
				DEBUGF("(int) ");
			} else if (vtype.vt.value == (uint16_t)VarType::Std::Float) {
				if (!unop_types_info[i].type_float) {
					add_08(I_OP_FTOI);
					type = VarType::Std::Int;
					DEBUGF("(float_to_int) ");
				} else {
					type = VarType::Std::Float;
					DEBUGF("(float) ");
				}
			}
...
VarType Code::binOpType(OpType optype, VarType left, VarType right, int left_pos) {
	...
	for (int i = 0; i < sizeof(binop_types_info) / sizeof(binop_types_info[0]); i++) {
		if (binop_types_info[i].type == optype) {
			// test
			if (left.vt.value == (uint16_t)VarType::Std::Int && right.vt.value == (uint16_t)VarType::Std::Int) {
				type = VarType::Std::Int;
				DEBUGF("(int int) ");
			} else if (left.vt.value == (uint16_t)VarType::Std::Int && right.vt.value == (uint16_t)VarType::Std::Float) {
				if (binop_types_info[i].float_float) {
					current_scope->fcode.insert(current_scope->fcode.begin() + left_pos, I_OP_ITOF);
					type = VarType::Std::Float;
					DEBUGF("(int_to_float float) ");
				} else {
					add_08(I_OP_FTOI);
					type = VarType::Std::Int;
					DEBUGF("(int float_to_int) ");
				}
			} else if (left.vt.value == (uint16_t)VarType::Std::Float && right.vt.value == (uint16_t)VarType::Std::Int) {
				if (binop_types_info[i].float_float) {
					add_08(I_OP_ITOF);
					type = VarType::Std::Float;
					DEBUGF("(float int_to_float) ");
				} else {
					current_scope->fcode.insert(current_scope->fcode.begin() + left_pos, I_OP_FTOI);
					type = VarType::Std::Int;
					DEBUGF("(float_to_int int) ");
				}
			} else if (left.vt.value == (uint16_t)VarType::Std::Float && right.vt.value == (uint16_t)VarType::Std::Float) {
				if (binop_types_info[i].float_float) {
					type = VarType::Std::Float;
					DEBUGF("(float float) ");
				} else {
					...
					add_08(I_OP_FTOI);
					type = VarType::Std::Int;
					DEBUGF("(float_to_int float_to_int) ");
				}
			}
...
VarType Code::expression(std::vector<Node> expr, std::vector<char*> _prefix, VarType expected_type, bool is_func_call, uint8_t _max_st) {
	...
	for (auto i = expr.begin(); i != expr.end(); i++) {
		switch ((NodeType)i->type) {
			...
			case NodeType::ConstFloat: {
				DEBUGF("[int]%f ", i->value.f);
				t_type.pos = current_scope->fcode.size() - 1;
				t_type.type = VarType::Std::Float;
			...
			case NodeType::ConstString: {
				DEBUGF("[string]%.64s ", i->value.c);
				t_type.pos = current_scope->fcode.size() - 1;
				t_type.type = VarType::Std::String;
			...
			case NodeType::Var: {
				...
				// Если есть префикс
				if (prefix.size() > 0) {
					if (variableHasParent(i->value.c)) {
						VarState par = getVariableParent(i->value.c);
						if (!compareStrings(par.name, i->value.c)) {
			...
			case NodeType::Operator: {
				...
				if (!IsUnaryOperatorType(i->value.op)) {
					...
					if (t_type.type.vt.value == (uint16_t)VarType::Std::None)
						errorTypeConvertion((VarType::Std)left.vt.value, (VarType::Std)right.vt.value);
					...
					// Если не удалось преобразовать тип
					if (t_type.type.vt.value == (uint16_t)VarType::Std::None)
						errorTypeConvertion((VarType::Std)etype.vt.value, VarType::Std::None);
					types_stack.back() = t_type;
				}
				if (types_stack.back().type.vt.value == (uint16_t)VarType::Std::Float)
					is_float = true;
			...
			case NodeType::Function: {
				if (func_data.empty() || func_data.back().name != i->value.c) {
					...
					else if (ret.vt.value == (uint16_t)VarType::Std::None)
					...
					VarType got_type = types_stack.back().type;
					VarType exp_type = expected.at(args_checked + 1).type;
					VarType v = typeConversion(got_type, exp_type);
					if (v.vt.value == (uint16_t)VarType::Std::None)
						errorWrongFunctionArgType(func_data.back().args.size() + 1, func_data.back().name);
					fas.type = v.vt;
			...
			case NodeType::FunctionNoArgs: {
				DEBUGF("%.64s() ", i->value.c);
				t_type.pos = current_scope->fcode.size() - 1;
				char *t_pref = (prefix.size() > 0) ? prefix.back() : nullptr;
				std::vector<FuncArgStruct> expected = getArgs(i->value.c, t_pref);
				if (expected.size() > 1) {
					if (expected.size() > 1)
						errorFunctionArgsCount(i->value.c, 0, expected.size() - 1);
				}
				
				VarType ret_type = getFunctionReturnType(i->value.c, t_pref);
				if (is_func_call && call_func_name == nullptr)
					call_func_name = i->value.c; // Первая функция не должна возвращать значение
				else if (ret_type.vt.value == (uint16_t)VarType::Std::None)
					errorNoReturnValue();
				functionCall(i->value.c, 0, t_pref);
				t_type.type = ret_type;
				types_stack.push_back(t_type);
				// Вызов функции временно занимает 4 байта для адреса функции
				if(!has_func) {
			...
			case NodeType::Delimeter: {
				...
				if (args_checked <= expected.size() - 2) {
					VarType v = typeConversion(types_stack.back().type,
								expected.at(args_checked + 1).type);
					if (v.vt.value == (uint16_t)VarType::Std::None)
						errorWrongFunctionArgType(func_data.back().args.size() + 1, func_data.back().name);
					fas.type = v.vt;
					func_data.back().args.push_back(fas);
				...
				// 	errorTooManyFunctionArgs(func_data.back().name);
				// }
				break;
			}
			case NodeType::Array: {
	...
	if (is_func_call && getFunctionReturnType(call_func_name, _prefix1.size() > 0 ? _prefix1.back() : nullptr).vt.value != (uint16_t)VarType::Std::None)
		add_08(I_POP);

	if (expected_type.vt.value != (uint16_t)VarType::Std::None && expected_type.vt.value != types_stack.back().type.vt.value) {
		DEBUGF("Результат выражения должен иметь тип '%.64s', а не '%.64s'",
...
VarType Code::typeConversion(VarType type, VarType expected) {
	if (type.vt.value == expected.vt.value) {
		return type;
	}
	else if (type.vt.value == (uint16_t)VarType::Std::Int && expected.vt.value == (uint16_t)VarType::Std::Float) {
		add_08(I_OP_ITOF);
		return VarType::Std::Int;
	} else if (type.vt.value == (uint16_t)VarType::Std::Float && expected.vt.value == (uint16_t)VarType::Std::Int) {
		add_08(I_OP_FTOI);
		return VarType::Std::Float;
	} else if (type.vt.value == (uint16_t)VarType::Std::Int && expected.vt.value == (uint16_t)VarType::Std::String) {
		functionCall((char*)"int_to_string", 1, (char*)nullptr);
		return VarType::Std::String;
	} else if (type.vt.value == (uint16_t)VarType::Std::Float && expected.vt.value == (uint16_t)VarType::Std::String) {
		functionCall((char*)"float_to_string", 1, (char*)nullptr);
		return VarType::Std::String;
	} else if (type.vt.value == (uint16_t)VarType::Std::Int && expected.vt.value == (uint16_t)VarType::Std::Character) {
		return VarType::Std::Character;
	}
	// Не удалось преобразовать тип
	errorTypeConvertion((VarType::Std)type.vt.value, (VarType::Std)expected.vt.value);
	return VarType::Std::None;
}
...
void Code::returnOperator(VarType type) {
	DEBUGF("ReturnOperator\n");
	char *name = current_scope->name;
	VarType expected, got;
	int l = findFunction(name);
	expected = declared_functions[l].args[0].type; // ожидаемый тип
	got = typeConversion((VarType::Std)type.vt.value, (VarType::Std)expected.vt.value);
	if (expected.vt.value == (uint16_t)VarType::Std::None) // Не возвращать значение
		add_08(I_OP_RETURN);
	...
	if (expected.vt.value != (uint16_t)VarType::Std::None) {
...
void Code::arrayDeclaration(char *name, VarType type, bool is_global) {
	...
	declareVariable(name, type, is_global);
...
VarType Code::loadArray(char *name) {
	...
	if (!IsArrayVarType(type))
		errorArrayType(name);
	bool isLocal;
	loadVariable(name, isLocal);
	switch ((VarType::Std)type.vt.value) {
		case VarType::Std::Int: {
			add_08(I_OP_IALOAD);
			return VarType::Std::Int;
		}
		case VarType::Std::Float: {
			add_08(I_OP_FALOAD);
			return VarType::Std::Float;
		}
		case VarType::Std::String: {
			if (type.vt.array) {
				add_08(I_OP_IALOAD);
				return VarType::Std::String;
			} else {
...
void Code::saveGotoLabel(char *label) {
	...
	for (int i = labels_count - current_scope->goto_labels; i < labels_count; i++) {
			...
			compareStrings(label, goto_labels.at(i).label))
		{
			...
			fcode_pos = goto_labels.at(i).position + 1;
			position = current - fcode_pos;
			// Безусловный переход
			current_scope->fcode.at(fcode_pos) = position & 0xFF;
			current_scope->fcode.at(fcode_pos + 1) = (position >> 8) & 0xFF;
...
void Code::gotoStatement(char *label) {
	...
	// Если метка не найдена, то может быть объявлена позже
	gs.is_dangling = 1;
	gs.label = label;
	gs.position = current_scope->fcode.size();
	goto_labels.push_back(gs);
...
int Code::streamCodeOutput(File file) {
	...
	file.write((const uint8_t*)&file_hdr, sizeof(file_hdr)); // Заголовок файла
