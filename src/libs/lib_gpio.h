...
static int GPIO_adc_read_millivolts(stack_t *&sp) {
    uint32_t pin = POP();
    //PUSH(analogReadMilliVolts(pin)); // ОШИБКА - исправить
    PUSH(0);
    return 1;
}
...
const struct lib_reg lib_gpio[] = {
    ...
    {"read", GPIO_read, 2, new VarType[2] {
        VarType::Std::Int, VarType::Std::Int,
    }}, //
    /* int adc_read(int pin) */
    {"adc_read", GPIO_adc_read, 2, new VarType[2] {
        VarType::Std::Int, VarType::Std::Int,
    }},
    /* int adc_read_millivolts(int pin) */
    {"adc_read_millivolts", GPIO_adc_read_millivolts, 2, new VarType[2] {
        VarType::Std::Int, VarType::Std::Int,
    }},
